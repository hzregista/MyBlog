...
class post {
    ...
    public function checklogin($username,$password){
     ...
     public function blogupdate($inf,$img){
         $who = functions::getid();
         $date = functions::date("int");
         if (empty($images)){
         $insert = $this->db->query("UPDATE tbblog SET hood = :h, title = :t, category =:ca, status =:s, content =:co WHERE id = :id", array("h" => $inf["hood"], "t" => $inf["title"], "ca" => $inf["category"], "s" => $inf["status"], "co" => $inf["content"], "id" => $inf["id"]));    
         }
         else {
         $insert = $this->db->query("UPDATE tbblog SET hood = :h, title = :t, category =:ca, status =:s, content =:co, images = :i WHERE id = :id", array("h" => $inf["hood"], "t" => $inf["title"], "ca" => $inf["category"], "s" => $inf["status"], "co" => $inf["content"], "id" => $inf["id"], "i" => $images));        
         }  
     }
